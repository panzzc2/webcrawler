...
#         exit();
    ...
    # http://www.kosmo.com.my/kosmo/cari.asp?paper=ko&dd=02&mm=11&yy=2008&query=Search&stype=dt
  
    urlList.append('http://www.kosmo.com.my/kosmo/cari.asp?paper=ko&dd=' + date[0] + '&mm=' + date[1] + '&yy=' + date[2] + '&query=Search&stype=dt')
    ...
    for url in urlList :
        ...
        try : 
            ...
            for returnedUrl in doc.xpath('//div[@class="search"]//ul//li//a') :
                #if not re.match('^http' , returnedUrl ):
                
                # return the search page in the form of [url, title, category, published date]
                url = 'http://www.kosmo.com.my' + str(returnedUrl.attrib.get('href'))
                if "&sec=" not in url:
...
def cleanResultFile(url):
    ...
    try :
            ...
            text = doc.xpath('//div[@id="ContentContainer"]//p')
